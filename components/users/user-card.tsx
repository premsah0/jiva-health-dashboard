...
export function UserCard({ user, onUpgrade, onView, onEdit }: UserCardProps) {
  ...
  return (
    <Link
      ...
      className="block rounded-xl border border-zinc-100 bg-white p-4 transition-all duration-200 hover:border-zinc-200 hover:shadow-sm dark:border-zinc-800 dark:bg-zinc-900/60 cursor-pointer group"
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        {/* 1. Initials Avatar and Core Info */}
        <div className="flex items-center gap-3.5 shrink-0 lg:w-[22%]">
          <div className="h-10 w-10 rounded-full bg-[#1a73e8] flex items-center justify-center text-white font-bold text-[13px] shrink-0 select-none group-hover:ring-2 group-hover:ring-blue-200 dark:group-hover:ring-blue-800 transition-all">
            {initials}
          </div>
          <div className="space-y-1 leading-tight">
            <h4 className="text-[15px] font-semibold text-[#263238] dark:text-zinc-50 tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
              {user.name}
            </h4>
        ...
        <div className="grid gap-1.5 border-t border-zinc-100 pt-3 text-[12px] leading-none text-zinc-500 lg:border-t-0 lg:pt-0 lg:w-[22%] dark:border-zinc-800">
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              className="font-medium hover:text-blue-600 transition-colors truncate max-w-[180px] sm:max-w-none cursor-pointer"
            >
              {user.email}
            </span>
          </div>
        ...
        <div className="flex flex-row justify-between items-center border-y border-zinc-100 py-3 lg:border-none lg:py-0 lg:flex-col lg:items-start lg:w-[13%] dark:border-zinc-800">
          ...
          <span className="text-lg font-bold text-blue-600 select-none leading-none pb-0.5 dark:text-blue-400">
        ...
        <div className="flex items-center justify-end gap-2 pt-1 lg:pt-0 lg:w-[22%]">
          {user.isPrime ? (
            ...
            <button
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); onUpgrade?.(user.id); }}
          ...
          {/* View Profile */}
          <button
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); onView?.(user.id); }}
          ...
          {/* Edit Profile */}
          <button
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); onEdit?.(user.id); }}
            className="h-[32px] px-2.5 border border-zinc-200 hover:border-zinc-300 dark:border-zinc-700 dark:hover:border-zinc-600 rounded-lg flex items-center justify-center gap-1 text-[12px] font-medium text-[#263238] dark:text-zinc-300 bg-white dark:bg-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer"
            title="Edit profile"
          >
